...
RELEASE_NAME="${3:-VICE-3.1-FRF-AmigaOS-PiStorm-2026-06-29-FINAL}"
...
for f in \
    frf_apply_final_ahi_timing_startup_fixes.sh \
    frf_build_complete_release_020_060_final_v3.sh \
...
echo
echo "Applying and validating final shared AHI fixes..."
bash "$ROOT/frf_apply_final_ahi_timing_startup_fixes.sh" "$ROOT" ||
    fail "Could not apply/validate final AHI timing and startup fixes"
...
timer = read("src/arch/amigaos/timer.c")
soundahi = read("src/sounddrv/soundahi.c")

# Final AHI timing and startup fixes.
add(
    "Correct AmigaOS timeval subtraction",
    "FRF_AHI_TIMER_SUBTRACT_FIX" in timer
    and "microseconds = dt->tv_usec - st->tv_usec;" in timer
    and "microseconds += 1000000;" in timer,
    "AHI elapsed time uses normal timeval subtraction and borrow",
)
add(
    "Malformed timer expression removed",
    "dt->tv_usec * (extrasub ==1 ) ? 10 : 1" not in timer,
    "old ternary precedence bug is absent",
)
add(
    "Real AHI backend resume hook",
    "FRF_AHI_BACKEND_RESUME_FIX" in soundahi
    and "return ahi_resume();" in soundahi,
    "VICE sound_resume() reopens the released AHI backend",
)
add(
    "No no-op AHI resume hook",
    not bool(re.search(
    ...
    "_ahi_resume() cannot falsely report success while AHI is closed",
)
...
for rel, text, token in [
    ("timer.c", timer, "FRF_AHI_TIMER_SUBTRACT_FIX"),
    ("soundahi.c", soundahi, "FRF_AHI_BACKEND_RESUME_FIX"),
...
lines.append("----------------")
for rel, text, token in [
    ("src/arch/amigaos/timer.c", timer, "FRF_AHI_TIMER_SUBTRACT_FIX"),
    ("src/sounddrv/soundahi.c", soundahi, "FRF_AHI_BACKEND_RESUME_FIX"),
...
bash "$ROOT/frf_build_complete_release_020_060_final_v3.sh" \
    "$ROOT" "$RELEASE_ROOT" ||
...
cp -v "$ROOT/RUNTIME_TEST_CHECKLIST.txt" \
    "$RELEASE_ROOT/RUNTIME_TEST_CHECKLIST.txt"
...
SOURCE_ZIP="$RELEASE_ROOT/Source/VICE-3.1-FRF-AmigaOS-PiStorm-Modified-Source-2026-06-29.zip"
...
Source/VICE-3.1-FRF-AmigaOS-PiStorm-Modified-Source-2026-06-29.zip contains the complete corresponding
