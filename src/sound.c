...
#include "vice.h"
static const char frf_sound_ahi_22050_441_8_final_marker[] = "FRF_SOUND_AHI_22050_441_8_FINAL";
...
int sound_open(void)
{
    ...
    if (pdev) {
        if (pdev->init) {
            ...
            /* FRF_SOUND_AHI_22050_441_8_FINAL_APPLY */
            speed = 22050;
            channels = 1;
            channels_cap = 1;
            fragsize = 441;
...
#if defined(AMIGA_SUPPORT)
        if (pdev->name != NULL && !strcasecmp(pdev->name, "ahi")) {
            speed = 22050;
            channels = 1;
            channels_cap = 1;
            snddata.sound_output_channels = 1;
            fragsize = 441;
            fragnr = 8;
            snddata.fragsize = 441;
            snddata.fragnr = 8;
            snddata.bufsize = snddata.fragsize * snddata.fragnr;
            /* FRF_SOUND_AHI_22050_441_8_FINAL_AFTER_INIT */
        }
#endif
...
#if defined(AMIGA_SUPPORT)
            if (snddata.playdev != NULL && snddata.playdev->name != NULL && !strcasecmp(snddata.playdev->name, "ahi")) {
                used = snddata.fragsize; /* FRF_AHI_NO_FALSE_DRAIN_WARN */
            } else
#endif
            {
