...
{
    nr /= num_channels; /* samples per channel */
    return ahi_play_samples(pbuf, nr, NOTIME, NOWAIT);
}
...
static int _ahi_resume(void)
{
     ...
     * Resume the existing queue; do not reopen the complete AHI backend.
     */
    ahi_continue();
    return 0;
}
