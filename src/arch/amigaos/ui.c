...
static const ui_menu_toggle_t toggle_list[] = {
    ...
    { "FullscreenEnabled", IDM_TOGGLE_FULLSCREEN } /* FRF_FULLSCREEN_RESOURCE_NAME_FIX */,
...
{
    /* FRF_PAL_RGB_V39_TARGET_SCREEN_VISUALINFO */
    struct Screen *menuscreen = NULL;
    int i, j;
    ...
    if (canvas == NULL
        ...
        || canvas->os->window->WScreen == NULL) {
        return -1;
    }

    menuscreen = canvas->os->window->WScreen;

    if (!(canvas->os->VisualInfo = GetVisualInfo(menuscreen, TAG_DONE))) {
        return -1;
    }
...
int ui_menu_handle(video_canvas_t *canvas, int idm)
{
    ...
    switch (idm) {
        ...
        case IDM_NATIVE_SCREENSHOT_SETTINGS:
            ...
            break;
        case IDM_FRF_SELECT_SCREENMODE:
            video_arch_fullscreen_select_asl_mode();
            break;
...
void ui_event_handle(void)
{
    ...
    do {
        ...
        for (canvas = canvaslist; canvas; canvas = canvas->next) {
            ...
            while ((imsg = (struct IntuiMessage *)GetMsg(canvas->os->window->UserPort))) {
                ...
                switch (imClass) {
                    case IDCMP_MENUPICK:
                        /* FRF_PAL_RGB_V39_MENUPICK_END */
                        video_arch_native_menu_end();
                        pointer_to_default();
                        ...
                        break;
                    /* FRF_PAL_RGB_V39_MENUVERIFY_BEGIN */
                    case IDCMP_MENUVERIFY:
                        video_arch_native_menu_begin();
                        break;
    ...
    if (do_quit_vice) {
        ...
        if (do_quit_vice) {
            ...
            exit(0); // stock VICE AmigaOS quit path
